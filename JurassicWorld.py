...
def show_graph(C, x_point=None, y_point=None, Optimal_x=None):
    ...
    if Optimal_x is not None:
        ...
        x3 = max(0, min(x_max, Optimal_x / 1.1))
        x4 = max(0, min(x_max, Optimal_x * 1.1))
    ...
    if x_point is not None and y_point is not None:
        ...
        ax.annotate(
            f"({x_point}, {y_point})\n{int(x_point * 100 / Optimal_x)}% ~ {int(Optimal_x * 100 / x_point)}%",
            (x_point, y_point),
            xytext=(5, 5),
...
def tab3():
    ...
    Attack3 = st.number_input("3rd Attack", min_value=0, value=int(remain_Fero/16), step=10)
