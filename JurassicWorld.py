...
def show_graph(C, x_point=None, y_point=None, Optimal_x=None):  
    ...
    if real_x > deadzone_x: ax.fill_between(x[(x >= x1) & (x <= deadzone_x_neo)], 0, y[(x >= x1) & (x <= deadzone_x_neo)], hatch='/', facecolor='none', edgecolor='black') # add "/" hatch for non-red region from 0→400
    # =========================  
    # VÙNG RED (> C)  
    # =========================  
...
def tab3():  
    ...
    st.markdown("---")  
    defaultATK3 = remain_Fero/9 if (remain_Fero/4.2 < 400) else 400
